  ...
  return info ? (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row text-white mt-8">
        <img className="h-auto sm:h-[50vh] object-cover rounded-lg" src={`https://image.tmdb.org/t/p/original/${info.detail.poster_path || info.detail.backdrop_path}`} alt="TV Poster" />
        ...
        <div className="content mt-6 sm:mt-0 sm:ml-8">
          <h1 className="text-3xl sm:text-5xl font-black">
            ...
            <small className="text-lg sm:text-2xl font-bold text-zinc-200"> ({info.detail.first_air_date.split('-')[0]})</small>
          </h1>
          ...
          <div className="mt-3 mb-5 flex flex-wrap items-center gap-4">
            ...
            <h1>{info.detail.episode_run_time ? `${info.detail.episode_run_time[0]} min` : ''}</h1>
          </div>
      ...
      <div className="w-full flex overflow-x-auto mb-5 p-5">
        {info.detail.seasons.map((s, i) => (
          <div key={i} className="w-[15vh] mr-8">
            <img className="shadow-lg min-w-[14vw] h-[30vh] object-cover" src={`https://image.tmdb.org/t/p/original/${s.poster_path}`} alt={s.name} />
            <h1 className="text-lg sm:text-2xl text-zinc-300 mt-3 font-semibold hover:text-[#F56009]">{s.name}</h1>
          </div>
        ))}
      </div>
      ...
      {/* Recommendations */}
      <hr className="mt-10 mb-5 border-none h-[2px] bg-zinc-500" />
      <h1 className="text-2xl sm:text-3xl font-bold text-white">TV Recommendations & Similar Shows</h1>
